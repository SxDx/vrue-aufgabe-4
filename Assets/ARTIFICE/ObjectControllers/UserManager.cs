...
public class UserManager : ScriptableObject
{
	...
	public void AddNewPlayer (NetworkPlayer player, bool isClient)
	{
		...
		GameObject gui = GameObject.Find ("GUIOBj");
		if (isClient) {
			gui.networkView.RPC ("setName", player, playerName);
		} else {
			gui.GetComponent<NetworkGUI> ().setPlayerName (playerName);
		}
		// ------------------ VRUE Tasks END ----------------------------
	}
	...
	public NetworkPlayer getNetworkPlayer (string playerName)
	{
		...
		foreach (DictionaryEntry playerEntry in players) {
			if (System.Convert.ToString (playerEntry.Value).Equals (playerName)) {
				return (NetworkPlayer)playerEntry.Key;
			}
		}
